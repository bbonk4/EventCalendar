  ...
  DateTime nextDateTime(DateTime start, {int position = 0}) {
    ...
    if (frequency == Frequency.hourly) {
      ...
      if (start == startDate ||
          !(start
                      .difference(
                          DateTime.fromMillisecondsSinceEpoch(startDate.millisecondsSinceEpoch + offset * HOUR_IN_MILLI)
                              .toUtc())
                      .inMilliseconds /
                  HOUR_IN_MILLI)
              .isNegative) {
    ...
    if (frequency == Frequency.weekly) {
      ...
      DateTime temp = nearestDay(
          start,
          List.generate(byDay.length, (position) {
//        print(byDay[position]);
      ...
      if (temp == null) {
        ...
        DateTime tempTime =
            DateTime.fromMillisecondsSinceEpoch(startDate.millisecondsSinceEpoch + interval * position * WEEK_IN_MILLI)
                .toUtc();
        temp = nearestDay(
            tempTime,
            List.generate(byDay.length, (position) {
    ...
    if (frequency == Frequency.yearly) {
      ...
      if (!start
              .difference(new DateTime.utc(
                  start.year, startDate.month, startDate.day, startDate.hour, startDate.minute, startDate.second))
              .isNegative ||
          start.difference(new DateTime.utc(
                  start.year, startDate.month, startDate.day, startDate.hour, startDate.minute, startDate.second)) ==
              Duration.zero) {
        offset += interval;
      }
      ...
      if (count >= offset / interval) {
        nextTime = DateTime.utc(startDate.year + offset, startDate.month, startDate.day, startDate.hour,
  ...
  DateTime nearestDay(DateTime start, List<DateTime> dates, {initial = false}) {
    ...
    for (int i = 0; i < dates.length; i++) {
      if (dif > -start.difference(dates[i]) && start.difference(dates[i]).isNegative ||
          (initial && start == dates[i])) {
        dif = -start.difference(dates[i]);
      }
    }

    if (dif.isNegative) return null;
...
//      print("negative");
      ...
      if (temp
              .difference(
                  new DateTime.utc(temp.year, temp.month, temp.day, startDate.hour, startDate.minute, startDate.second))
              .isNegative ||
          initial) {
...
//      print("positive");
  ...
  int getMonthsApart(DateTime start, DateTime end) {
    ...
    if (isLastDayOfMonth) {
      while (start.difference(end).isNegative) {
        ...
        start = new DateTime(start.year, start.month, lastDayOfMonth(start), start.hour, start.minute, start.second)
            .add(Duration(days: 1));
      }
    } else {
      int day = start.day;
  ...
  Result<List<DateTime>> between(DateTime start, DateTime end) {
    ...
    while (time != null &&
        (time.difference(end).isNegative || time.difference(end) == Duration.zero) &&
        (until == null ||
            (until != null &&
                time != until &&
                (time.difference(until).isNegative || time.difference(until) == Duration.zero)))) {
      times.add(time);
  ...
  factory RecurrenceRule.fromJson(Map<String, dynamic> json) {
    ...
    return RecurrenceRule(Frequency.values[json['frequency']],
